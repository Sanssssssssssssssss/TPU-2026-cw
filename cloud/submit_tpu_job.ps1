...
.\cloud\submit_tpu_job.ps1 submit-r12-tail-lr3e6-cos-g512 -RunId r12-tail-lr3e6-cos-g512-001
...
function Submit-R12TailLr3e6CosG512 {
    Assert-RunId
    $bundle = New-CodeBundle
    try {
        $runner = Upload-Runner
        $remoteBundle = Upload-Bundle $bundle
        $remoteSecrets = Upload-SecretsIfPresent
        Invoke-RemoteRunner $runner "submit-r12-tail-lr3e6-cos-g512" $remoteBundle $remoteSecrets
    } finally {
        Remove-CodeBundle $bundle
    }
}
...
switch ($Command) {
    ...
    "submit-r12-tail-lr3e6-cos-g512" { Submit-R12TailLr3e6CosG512 }
