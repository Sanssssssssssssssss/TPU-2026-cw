...
submit_r12_high_rank_pilot() {
  ...
  bootstrap_env
  check_tpu_backend
  write_k8_pilot_script "R12_rank128_alpha128_beta004_lr2e-6:gsm8k_verifiable_simple:0.04:2e-6:128:128:0.2 R12_rank128_alpha64_beta004_lr2e-6:gsm8k_verifiable_simple:0.04:2e-6:128:64:0.2"
  ...
  echo "==> Starting tmux session $session"
  tmux new-session -d -s "$session" "K8_MAX_STEPS=256 K8_LR_SCHEDULE_STEPS=841 K8_WARMUP_STEPS=84.1 K8_CHECKPOINT_STEPS='32 64 96 128 160 192 224 256' K8_MAX_TO_KEEP=12 K8_SAVE_INTERVAL_STEPS=32 K8_EVAL_EVERY_N_STEPS=32 K8_NUM_GENERATIONS=8 K8_RANK=128 K8_ALPHA=128 K8_LEARNING_RATE=2e-6 K8_BETA=0.04 K8_EPSILON=0.2 bash '$RUN_DIR/run_k8_pilot.sh' 2>&1 | tee -a '$RUN_DIR/pipeline.log'; status=\${PIPESTATUS[0]}; echo; echo \"--- k8 R12 high-rank pilot exited (\$status) ---\"; exec bash"
  echo "Started R12 high-rank K8 pilot. Attach with: tmux attach -t $session"
  echo "Log: $RUN_DIR/pipeline.log"
}
...
submit_r12_r64_lr_smoothing() {
  require_run_id
  unpack_bundle
  install_secrets
  bootstrap_env
  check_tpu_backend
  write_k8_pilot_script "R12_r64_beta004_lr2e-6_eps02:gsm8k_verifiable_simple:0.04:2e-6:64:64:0.2 R12_r64_beta005_lr2e-6_eps02:gsm8k_verifiable_simple:0.05:2e-6:64:64:0.2"
  ...
  echo "==> Starting tmux session $session"
  tmux new-session -d -s "$session" "K8_MAX_STEPS=256 K8_LR_SCHEDULE_STEPS=841 K8_WARMUP_STEPS=84.1 K8_CHECKPOINT_STEPS='32 64 96 128 160 192 224 256' K8_MAX_TO_KEEP=12 K8_SAVE_INTERVAL_STEPS=32 K8_EVAL_EVERY_N_STEPS=32 K8_NUM_GENERATIONS=8 K8_RANK=64 K8_ALPHA=64 K8_LEARNING_RATE=2e-6 K8_BETA=0.04 K8_EPSILON=0.2 bash '$RUN_DIR/run_k8_pilot.sh' 2>&1 | tee -a '$RUN_DIR/pipeline.log'; status=\${PIPESTATUS[0]}; echo; echo \"--- k8 R12 R64 LR smoothing exited (\$status) ---\"; exec bash"
  echo "Started R12 R64 LR smoothing K8 pilot. Attach with: tmux attach -t $session"
  echo "Log: $RUN_DIR/pipeline.log"
}
...
submit_r12_r64_small_beta_tuning() {
  require_run_id
  unpack_bundle
  install_secrets
  bootstrap_env
  check_tpu_backend
  write_k8_pilot_script "R12_r64_beta0001_lr3e-6_eps02:gsm8k_verifiable_simple:0.001:3e-6:64:64:0.2 R12_r64_beta000_lr3e-6_eps02:gsm8k_verifiable_simple:0.0:3e-6:64:64:0.2"
  ...
  echo "==> Starting tmux session $session"
  tmux new-session -d -s "$session" "K8_MAX_STEPS=256 K8_CHECKPOINT_STEPS='32 64 96 128 160 192 224 256' K8_MAX_TO_KEEP=12 K8_SAVE_INTERVAL_STEPS=32 K8_EVAL_EVERY_N_STEPS=32 bash '$RUN_DIR/run_k8_pilot.sh' 2>&1 | tee -a '$RUN_DIR/pipeline.log'; status=\${PIPESTATUS[0]}; echo; echo \"--- k8 r64 small-beta tuning pilot exited (\$status) ---\"; exec bash"
  echo "Started R12 R64 small-beta tuning pilot. Attach with: tmux attach -t $session"
  echo "Log: $RUN_DIR/pipeline.log"
}
...
case "$COMMAND" in
  ...
  submit-r12-r64-beta-clip-tuning)
    submit_r12_r64_beta_clip_tuning
    ;;
  submit-r12-r64-small-beta-tuning)
    submit_r12_r64_small_beta_tuning
    ;;
